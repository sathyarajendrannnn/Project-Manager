...
const LeaveRequests = () => {
  ...
  const addLeave = () => {
    if (!form.name || !form.from || !form.to) return alert("Please fill all fields");
    setLeaves([...leaves, { ...form, id: Date.now() }]);
  ...
  return (
    <div className="p-6 space-y-8 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-blue-800">Leave Management</h1>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
        <h2 className="text-xl font-semibold mb-4 text-blue-700 flex items-center gap-2">
          <PlusCircle size={20} /> Apply Leave
        </h2>
        <div className="grid grid-cols-5 gap-3">
          ...
          >
            <PlusCircle size={16} /> Submit Leave
          </button>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-blue-700">Leave Requests</h2>
        </div>
          ...
          <tbody>
            {filtered.map((l) => (
              <tr key={l.id} className="border-b hover:bg-blue-50">
                ...
                <td className="p-2">
                  <span className={`px-3 py-1 text-sm rounded-full ${statusColors[l.status]}`}>
                    {l.status}
                  </span>
                </td>
                <td className="p-2">
